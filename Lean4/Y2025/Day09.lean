...
def parse_line := (fun (a b : Nat) ↦ (b, a)) <$> number <* pchar ',' <*> number
def parse_input := separated parse_line eol

def parse : String → Option (Array (Nat × Nat)) := AoCParser.parse parser
  where
    parser : Parser (Array (Nat × Nat)) := parse_input

end parser

namespace Part1

def solve (input : Array (Nat × Nat)) : Nat :=
  input.iter
    |>.enumerate
    |>.map (fun (i, p) ↦
      input.iterFromIdx (i + 1)
        |>.map (fun (q : Nat × Nat) ↦ (dist₁ p.1 q.1) * (dist₁ p.2 q.2))
        |>.fold max 0)
    |>.fold max 0
...
/--
  ...
  - 2 : filled
  - 3 : unvisited -/
def solve (input : Array (Nat × Nat)) : Nat := Id.run do
  let ys := input.iter |>.map (·.fst) |>.toList |> HashSet.ofList |>.toArray |>.push 0 |>.qsort
  let encodeY := ys.iter.enumerate.map (fun (i, d) ↦ (d, i)) |>.toList |> HashMap.ofList
  if encodeY.size == 0 then return 0
  let xs := input.iter |>.map (·.snd) |>.toList |> HashSet.ofList |>.toArray |>.push 0 |>.qsort
