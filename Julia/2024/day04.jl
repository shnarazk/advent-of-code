...
function part1(m::Matrix{Char})::Int
    count(
        map(
            ...
            [(i, j) for i = 1:size(m, 1) for j = 1:size(m, 2)-3],
        ),
    ) +
    count(
        map(
            ((i, j),) -> (s = String(m[i:i+3, j]); s == "XMAS" || s == "SAMX"),
            [(i, j) for i = 1:size(m, 1)-3 for j = 1:size(m, 2)],
        ),
    ) +
    count(
        map(
            ((i, j),) -> (
                s = String([m[i, j], m[i+1, j+1], m[i+2, j+2], m[i+3, j+3]]); s == "XMAS" ||
                    s == "SAMX"
            ),
            [(i, j) for i = 1:size(m, 1)-3 for j = 1:size(m, 2)-3],
        ),
    ) +
    count(
        map(
            ...
            [(i, j) for i = 1:size(m, 1)-3 for j = 4:size(m, 2)],
        ),
    )
end
...
function part2(m::Matrix{Char})::Int
    count(
        ...
        [(i, j) for i = 2:size(m, 1)-1 for j = 2:size(m, 2)-1],
    )
end
